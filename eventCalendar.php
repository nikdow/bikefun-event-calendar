...
*/
	...
	if ( $previous ) {
		$calendar_output .= "\n\t\t".'<td colspan="3" id="prev"><a href="' . add_query_arg( array('calyear'=>$previous->year, 'calmonth'=>$previous->month ), 'https://www.bikefun.org' ) . '" title="' . esc_attr( sprintf(__('View events for %1$s %2$s'), $wp_locale->get_month($previous->month), date('Y', mktime(0, 0 , 0, $previous->month, 1, $previous->year)))) . '">&laquo; ' . $wp_locale->get_month_abbrev($wp_locale->get_month($previous->month)) . '</a></td>';
	} else {
	...
	if ( $next ) {
		$calendar_output .= "\n\t\t".'<td colspan="3" id="next"><a href="' . add_query_arg( array('calyear'=>$next->year, 'calmonth'=>$next->month ), 'https://www.bikefun.org' ) . '" title="' . esc_attr( sprintf(__('View events for %1$s %2$s'), $wp_locale->get_month($next->month), date('Y', mktime(0, 0 , 0, $next->month, 1, $next->year))) ) . '">' . $wp_locale->get_month_abbrev($wp_locale->get_month($next->month)) . ' &raquo;</a></td>';
	} else {
  ...
  $query = $wpdb->get_results( "SET time_zone = '" .  get_option('timezone_string') . "'" );
...
class Event_Calendar extends WP_Widget {
	...
	function widget( $args, $instance ) {
        ...
        if ( $post && ! isset( $_GET['calyear'] ) ) { // show calendar for month of this event
          ...
          if( array_key_exists("bf_events_startdate", $custom)) {
